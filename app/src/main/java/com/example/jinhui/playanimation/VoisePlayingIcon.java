...
public class VoisePlayingIcon extends View {
    ...
    public VoisePlayingIcon(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        //取出自定义属性
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.voisePlayingIconAttr);
        pointerColor = typedArray.getColor(R.styleable.voisePlayingIconAttr_pointer_color, Color.RED);
        pointerNum = typedArray.getInt(R.styleable.voisePlayingIconAttr_pointer_num, 4);//指针的数量，默认为4
        pointerWidth = DensityUtils.dp2px(getContext(),
                typedArray.getFloat(R.styleable.voisePlayingIconAttr_pointer_width, 5f));//指针的宽度，默认5dp
        pointerSpeed = typedArray.getInt(R.styleable.voisePlayingIconAttr_pointer_speed, 40);
    ...
    public VoisePlayingIcon(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.voisePlayingIconAttr);
        pointerColor = typedArray.getColor(R.styleable.voisePlayingIconAttr_pointer_color, Color.RED);
        pointerNum = typedArray.getInt(R.styleable.voisePlayingIconAttr_pointer_num, 4);
        pointerWidth = DensityUtils.dp2px(getContext(),
                typedArray.getFloat(R.styleable.voisePlayingIconAttr_pointer_width, 5f));
        pointerSpeed = typedArray.getInt(R.styleable.voisePlayingIconAttr_pointer_speed, 40);
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < pointers.size(); i++) {
            //绘制指针，也就是绘制矩形
            canvas.drawRect(basePointX, basePointY - pointers.get(i).getHeight(),
                    basePointX + pointerWidth, basePointY, paint);
